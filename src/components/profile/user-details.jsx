...
export default function UserDetails({ username, loggedInUser, userObj, user }) {
    const [isfollowing, setIsFollowing] = useState(
        user?.following.includes(userObj.userId)
    );
        ...
        <div className="flex relative pt-4 sm:pt-20 pb-36 md:pb-12 lg:w-935 mx-auto pl-4 md:pl-16">
            ...
            <div className="flex flex-col">
                <div className="flex mb-5 mt-1">
                    ...
                    {loggedInUser.displayName != username && user && (
                        <button
                            onClick={handleFollowUser}
                            className="w-20 md:w-24 text-sm h-8 bg-blue absolute left-30 md:static top-16 rounded-sm text-white font-semibold"
                        >
                            {isfollowing === null
                                ? user && user.following.includes(userObj?.userId)
                                    ? "Unfollow"
                                    : "Follow"
                                : isfollowing
                                    ? "Unfollow"
                ...
                <div className="flex items-center justify-between px-10 md:px-0 absolute left-0 bottom-0 w-screen md:w-auto h-16 border-t-1 border-b-1 md:border-none border-gray-300 md:static md:mb-5 text-base">
                    <p className="text-center text-gray-400 md:mr-10">
                        <strong className="font-semibold text-black">0</strong>{" "}
                        <br className="lg:hidden" /> posts
          </p>
                    <p className="text-center text-gray-400 md:mr-10">
                        <strong className="font-semibold text-black">
                            {followersCount}
                        </strong>
                        <br className="lg:hidden" /> followers
          </p>
                    <p className="text-center text-gray-400 md:mr-10">
                        <strong className="font-semibold text-black">
                            {userObj.following.length}
                        </strong>
                        <br className="lg:hidden" /> following
          </p>
                </div>
                <h3 className="font-semibold absolute left-4 sm:left-28 top-32 sm:top-44 md:static">
                    {userObj.fullName}
                </h3>
            </div>
        </div>
