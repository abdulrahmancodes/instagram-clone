        ...
        <>
            {comments.length > 2 ? (
                <p className="text-gray-400 text-sm pl-4">
                    View all{" "}
                    {comments.length + (commentsAdded ? commentsAdded.length : 0)}{" "}
          comments
                </p>
            ) : null}
            <div className="flex flex-col">
                {comments &&
                    comments
                        .slice(0, 2)
                        .map((comment) => (
                            <Comment comment={comment} key={comment.displayName} />
                        ))}
                {commentsAdded &&
                    commentsAdded.map((obj) => (
                        <Comment comment={obj} key={obj.displayName} />
                    ))}
            </div>
            <span className="text-xs text-gray-400 ml-4 my-1">
