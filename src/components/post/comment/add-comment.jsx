...
export default function AddComment( {commentsAdded, setCommentsAdded, docId, loggedInUser} ) {
    ...
    function postComment(e) {
        ...
        commentsAdded ? setCommentsAdded([...commentsAdded, commentObj]) : setCommentsAdded([commentObj]);
        setCommentText('');
    }
    ...
    return (
        <>
            <form method="POST" onSubmit={postComment} className="flex p-4 items-center border-t-1 border-gray-200" >
                ...
                <input type="text" id="input" placeholder="Add a comment..." className="w-11/12 outline-none caret-black text-sm placeholder-gray-300-gray-300 ml-4" onChange={(e) => setCommentText(e.target.value)} value={commentText} />
                <button disabled={commentText ? false : true} type="submit" className={`font-semibold text-sm text-blue ${commentText ? "opacity-100" : "opacity-50"} ml-4`}>Post</button>
            </form>
        </>
    )
}
